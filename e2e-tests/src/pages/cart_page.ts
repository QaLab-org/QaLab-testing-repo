...
const selectors = {
  ...
  removeFromCartSauceLabsBackpack:
...
export class CartPage extends BasePage {
  ...
  async addProductToCart(name: string): Promise<void> {
    const selector = `[data-test="add-to-cart-${name
      .toLowerCase()
      .split(" ")
      .join("-")}"]`;
  ...
  async verifyProductInCart(name: string): Promise<void> {
    ...
    const currentURL: string = this.page.url();
    expect(currentURL).toContain(this.baseURL + "/cart.html");
    const inventoryName: string = await this.page
      .locator(selectors.inventoryItemName)
      .innerText();
    expect(inventoryName).toEqual(name);
  }

  async goToCheckoutPage(): Promise<void> {
    if (
      await this.page.locator(selectors.addToCartSauceLabsBackpack).isVisible()
    ) {
  ...
  async verifyNavigatedToCheckoutPage(): Promise<void> {
    ...
    await expect(currentURL).toContain(
      this.baseURL + "/checkout-step-one.html"
    );
  }
}
